...
export function SiteShell({children}:{children:ReactNode}) {
 ...
 <Sheet open={menu} onOpenChange={setMenu}><SheetContent className="mobile-nav-sheet mobile-nav-modern translate-x-0 translate-y-0"><SheetTitle>Where to next?</SheetTitle><SheetDescription>Choose your next step with Credit Kaaran.</SheetDescription><nav aria-label="Mobile navigation">{[{href:"/",label:"Home"},...nav,{href:"/about",label:"About Arvind"}].map((n,i)=><Link href={n.href} key={n.href} onClick={e=>{setMenu(false);if(n.href==="/cards"){e.preventDefault();openWallet("cards");}}}><span className="nav-number">0{i+1}</span>{n.label}<ArrowUpRight size={18}/></Link>)}</nav></SheetContent></Sheet>
